    ...
    <div class="mb-8 grid gap-6">
      <section class="card min-w-0 overflow-hidden">
        ...
        <div class="grid items-center gap-6 md:grid-cols-[minmax(260px,0.8fr)_minmax(0,1.2fr)]">
          <div class="mx-auto aspect-square w-full max-w-[280px] sm:max-w-[360px]">
            <svg viewBox="0 0 200 200" class="block h-full w-full">
          ...
          <div class="grid w-full gap-2 text-sm text-gray-700 sm:grid-cols-2 lg:grid-cols-3">
            <div v-for="segment in nutritionSegments" :key="segment.categorie" class="flex min-w-0 items-center gap-3 rounded-lg bg-slate-50 px-3 py-2">
              <span class="h-3.5 w-3.5 shrink-0 rounded-full" :style="{ backgroundColor: segment.color }" />
      ...
      <section class="card min-w-0 overflow-hidden">
        ...
        <div class="rounded-3xl bg-slate-50 p-4">
          ...
          <svg viewBox="0 0 560 110" preserveAspectRatio="none" class="block h-auto min-h-[9rem] w-full sm:min-h-[15rem]">
          ...
          <div class="mt-3 grid w-full grid-cols-7 gap-1 text-[9px] text-gray-500 sm:text-[11px]">
            <span v-for="(label, index) in attendanceTrendLabels" :key="`${label}-${index}`" class="min-w-0 truncate text-center">{{ label }}</span>
          </div>
          ...
          <div class="mt-4 rounded-2xl border border-slate-200 bg-white p-3 text-xs text-slate-700 shadow-sm">
            ...
            <div class="grid grid-cols-3 gap-2 font-medium text-slate-500">
              <div class="min-w-0 break-words">{{ i18n.t('dashboard.charts.dateLabel') }}</div>
              <div class="min-w-0 break-words">{{ i18n.t('dashboard.charts.attendanceLabel') }}</div>
              <div class="min-w-0 break-words">{{ i18n.t('dashboard.charts.consumptionLabel') }}</div>
            </div>
            <div class="mt-2 space-y-2 text-sm">
              ...
              >
                ...
                <span class="min-w-0 break-words">{{ row.taux }} %</span>
                <span class="min-w-0 break-words">{{ consumptionTrendSeries[index]?.value ?? 0 }}</span>
              </div>
            </div>
          </div>
        </div>
