...
<template>
  <div>
    <PageHeader
      ...
      subtitle="Enregistrer des fournisseurs et les associer à des produits (S3)."
    />

    <div class="card mb-6 grid gap-4 lg:grid-cols-2">
      <div>
        <label class="label">Nom</label>
        <input v-model="nom" class="input" placeholder="Coopérative Ambovombe" />
      </div>
      <div>
        <label class="label">Contact</label>
        <input v-model="contact" class="input" placeholder="Rabe +261 34 12 345 67" />
      </div>
      <div class="lg:col-span-2">
        <p class="label">Produits fournis</p>
    ...
    <div class="mb-4 flex items-center justify-between gap-4">
      <h2 class="text-lg font-semibold">Fournisseurs enregistrés</h2>
      <input v-model="search" class="input max-w-xs" placeholder="Rechercher un fournisseur…" />
    </div>
      ...
      <table class="w-full text-sm">
        ...
        <tbody>
          ...
          <tr v-if="!fournisseurOptions.length">
            <td colspan="3" class="px-5 py-6 text-center text-gray-500">Aucun fournisseur trouvé.</td>
          </tr>
        </tbody>
      </table>
    </div>
